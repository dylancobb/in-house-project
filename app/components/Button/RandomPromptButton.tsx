...
const RandomPromptButton = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      console.error(
        'Error fetching random sentence:',
        error.message,
        error.response
      )
    }
  ...
  return (
    <>
      <button
        onClick={handleClick}
        className='flex items-center gap-2 py-1 px-3 h-15 w-50 bg-green rounded-md shadow-md shadow-dark_blue text-dark_blue'
      >
